...
void menu::cheats_tab() {
    ...
    imwrap::checkbox("Epilepsy", opt::cheat::blinkcolor, "Changing skins");
    ...
    ImGui::NextColumn();
    imwrap::checkbox("Test Path Finding", opt::cheat::testmftiles, "Testing path finding.");

    ImGui::NextColumn();
    imwrap::checkbox("Dance Move", opt::cheat::dancemove, "Dancing while move");

    ImGui::NextColumn();
    imwrap::checkbox("Ghost", opt::cheat::ghost, "Ghost");

    ImGui::NextColumn();
    imwrap::checkbox("Nazi Smoke", opt::cheat::nazismoke, "Its not visual. And funny i love it :D");

    imwrap::prep_columns(6);
    ImGui::NextColumn();
    imwrap::checkbox("Only Crystal", opt::cheat::crystalmode, "IDK.");
    ...
    // anasini sikim yaddan cixarma bu terefi

    ImGui::NextColumn();
    imwrap::checkbox("No Name", opt::cheat::noname, "No name");
    ...
    imwrap::prep_columns(6);
    ImGui::NextColumn();
    ImGui::Checkbox("Sum LAGGER(GTPS)", &onlyforgtps);
    if (onlyforgtps) {
        static types::time times = std::chrono::system_clock::now();
        auto it = sdk::GetGameLogic()->GetNetObjMgr()->players.begin();
        std::advance(it, rand() % sdk::GetGameLogic()->GetNetObjMgr()->players.size());
    ...
    if (ImGui::Button("Fake Respawn(Bannable) (not visual)")) { // ReDesigned
        GameUpdatePacket packet{ 0 };
        packet.flags = 2308;
                             //Punch: 2560
                             //Respawn: 2308
   ...
   ImGui::SameLine();
   if (ImGui::Button("Got Hit By Electric (not visual)")) {
       GameUpdatePacket packet{ 0 };
